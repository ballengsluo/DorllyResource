...
namespace ResWeb.Controllers
{
    public class UploadController : Controller
    {
        ...
        [HttpPost]
        public JsonResult SaveImage(FormCollection collection)
        {
            try
            {
                //判断上传文件的a数目
                if (Request.Files.Count > 0)
                {
                    ...
                    if (filetypes[1] == "jpg" || filetypes[1] == "gif" || filetypes[1] == "png" || filetypes[1] == "bmg" || filetypes[1] == "jpeg")
                    {
                        ...
                        string path = "~/Content/public/" + regex.Replace(product, "");
                        ...
                        if (_imgServcie.Add(img))
                        {
                            ...
                            return Json(new { result = "1", msg = "图片保存成功！", path = path, id = imgID, tempid = tempID });
                        }
                        else
