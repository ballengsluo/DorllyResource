    ...
    public class StatisticsADController : BaseController
    {
        ...
        public FileResult ImportToExcel(string park, string stime, string etime, int model)
        {
            try
            {
                ...
                {
                    //汇总数据导出
                    fileName = "广告位出租率汇总报表-" + DateTime.Now.ToString("yyyyMMddHHmmsss");
                    ...
                    title.CreateCell(1).SetCellValue("广告位总数");
                ...
                {
                    //趋势图数据导出
                    fileName = "广告位出租率趋势报表-" + DateTime.Now.ToString("yyyyMMddHHmmsss");
                    if (string.IsNullOrEmpty(stime) || string.IsNullOrEmpty(etime))
                    {
                    ...
                    title.CreateCell(0).SetCellValue("园区");
                    var monthList = result.GroupBy(a => a.MonthTime).OrderBy(a => a.Key).Select(a => a.Key).ToList();
                    foreach (var item in monthList)
                    {
